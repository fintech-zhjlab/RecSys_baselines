...
if __name__ == '__main__':
    ...
    pop_ratios_train = get_pop_ratios(train)
    pop_ratios_test = get_pop_ratios(test)
    ...
    user_varlen_feature_columns = [VarLenSparseFeat(SparseFeat('user_hist', vocabulary_size=3760, embedding_dim=4),
    ...
    model = DCCL(user_feature_columns,
    ...
    pred_ts = model.predict(test_model_input,
                            pop_ratios_test,
                            batch_size=2000)
    y_test = test[target].values.squeeze()
    print("test LogLoss", round(log_loss(y_test, pred_ts), 4))
    print("test AUC", round(roc_auc_score(y_test, pred_ts), 4))
    ...
    torch.save(test[target].values.squeeze(), "save/labels.pt")

    # 注释：user tower
    model_user = DCCL(user_feature_columns,
                      [],
                      task='binary',
                      device=device)
    ...
    torch.save(user_embedding, "save/dccl_users.pt")
    save_user_hist = test_user_hist.reshape([-1])
    save_user_hist = save_user_hist[save_user_hist != 0]
    torch.save(save_user_hist, "save/pos_items.pt")
    ...
    item_embedding = model_item.predict(item_model_input,
                                        pop_ratios_test,
                                        batch_size=2000)
    torch.save(item_embedding, "save/dccl_items.pt")
