...
if __name__ == '__main__':
    ...
    # target = ['rating']
    sparse_features = ['user_id', 'movie_id', 'occupation', 'genres']
    dense_features = ['user_mean_rating', 'item_mean_rating']

    user_atom_sparse_features = ['user_id']
    user_side_sparse_features = ['occupation']
    user_side_dense_features = ['user_mean_rating']

    item_atom_sparse_features = ['movie_id']
    # item_side_varlen_features = ['genres']
    item_side_dense_features = ['item_mean_rating']
    ...
    user_atom_columns = [SparseFeat(feat, data[feat].nunique(), embedding_dim=4) for i, feat in
                         enumerate(user_atom_sparse_features)]
    user_side_columns = [SparseFeat(feat, data[feat].nunique(), embedding_dim=4) for i, feat in
                         enumerate(user_side_sparse_features)] + [DenseFeat(feat, 1, ) for feat in
                                                                  user_side_dense_features]

    item_atom_columns = [SparseFeat(feat, data[feat].nunique(), embedding_dim=4) for i, feat in
                         enumerate(item_atom_sparse_features)]
    item_side_varlen_feature_columns = [VarLenSparseFeat(SparseFeat('genres', vocabulary_size=1000, embedding_dim=4),
                                                         maxlen=genres_maxlen, combiner='mean', length_name=None)]
    item_side_columns = item_side_varlen_feature_columns + [DenseFeat(feat, 1, ) for feat in
                                                                               item_side_dense_features]
    ...
    # user_feature_columns = user_atom_columns + user_side_columns
    # item_feature_columns = item_atom_columns + item_side_columns
